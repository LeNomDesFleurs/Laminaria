...
use crossterm::{cursor, event, event::Event,event::KeyModifiers, event::KeyCode, event::KeyEvent, style::Stylize, terminal,
    terminal::disable_raw_mode, terminal::enable_raw_mode,
};
...
pub fn interaction(
    ...
    let mut _midi_connection = match connect_midi(
        ...
        { gui_sender.send(UiEvent::UpdateMidiportName(port_name)).unwrap();
            midi_connection},
...
pub fn ui(parameters: Arc<Mutex<Parameters>>, receive_event: Receiver<UiEvent>) -> Result<()> {
    ...
    loop {
        ...
        match event {
            ...
            UI::UpdateMidiportName(port_name) => {midi_port_name = port_name; midi_channel = 0},
            UI::Refresh => {},
...
fn update_display(parameters: &Vec<Parameter>, midi_channel: u8, midi_port_name: &String, selected: i32, top_selection_index: i32, size: i32) {
    println!("{}", terminal::Clear(terminal::ClearType::All));
    println!("{}", cursor::MoveTo(0, 0));
